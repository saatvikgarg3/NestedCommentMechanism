    ...
    return <>
        <div className="comment">
            ...
            <div className="footer">
                <IconBtn onClick={onToggleCommentLike} disabled={toggleCommentLikeFn.loading} Icon={ likedByMe? FaHeart:FaRegHeart} aria-label={likedByMe?"unlike":"Like"}>{likeCount}</IconBtn>
                <IconBtn onClick={()=>setisReplying(prev=>!prev)} isActive={isReplying} Icon={FaReply} aria-label={isReplying?"Cancel Reply":"Reply"}/>
                {user.id=== currentuser.id && (
                  <>
                    <IconBtn onClick={()=>setisEditing(prev=>!prev)} isActive={isEditing} Icon={FaEdit} aria-label={isReplying?"Cancel Edit":"Edit"}/>
                    <IconBtn disabled={deleteCommentFn.loading} onClick={onCommentDelete} Icon={FaTrash} aria-label="Delete" color="danger"/>
                  </>
                )}
            </div>
